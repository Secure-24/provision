...
PCB
  ...
  if box_url
    box_url_full_path = File.expand_path(box_url)
    raise "box_url does not exist: #{box_url_full_path}" unless File.exist?(box_url_full_path)
    box_url_config = "config.vm.box_url = '#{box_url_full_path}'"
  else
    box_url_config = ''
  end
  vf = <<-VF
